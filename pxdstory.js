	...
	function show_profile(){
      ...
      if(data){
			...
			var profile_div=$(".profile");
				profile_div.append($("<div class=profile_text><span class=name>"+json.name+"</span><span class=desc>"+json.desc+"</span></div>"));
				
				let url=json.img;
				if(url){
					if(!url.match("^http")) url="http:"+url;
				var img=$("<div>").css("background-image","url('"+url+"')").addClass("profile_img");
...
function new_article_list_author(){
	...
	$(".list_type_notice li a").each((i,element)=>{
		let article_id=$(element).attr("href").match(/[0-9]+/)[0];
      ...
      if(!data) return
			...
			$(".tag_article_list").each((i, element) => {
				...
				articles.forEach((article,i) => {
					...
					var li=$("<li>");
					li.html("<a href=/"+no+"><img class=thumb src='"+url+"'><div class=info><span class=title>"+title+"</span> <br><span class=author>"+author+"</span> <span class=date>"+date+"</span></div> </a><br clear=both> ");
					//console.log(li);
					if(i>=5) li.css("display","none");
					element.append(li[0]);
				});
				if(articles.length>5){
					let more_button=$("<span class=more_button>").text(articles.length+"개 모두 보기")
					.click(function(){$(this).parent().children("li").show();
													 $(this).hide();})
...
function show_author_article(){
      ...
      if(data){
		...
		var article_list_div=$("#author_article_list");
				...
				for(var i=0;i<json.length;i++){
					...
					li.html("<a href=/"+no+"><img class=thumb src='"+url+"'><span class=title>"+title+"</span> <br><span class=date>"+date+"</span></a><br clear=both> ");
	...
	function show_reference_tag(){
		...
		tags.forEach(tag => {
			var article_list_div = $("<ul>")
				.attr("tag", tag)
				.addClass("article_list")
				.addClass("tag_article_list");
	...
	function remove_private_category(){
		...
		var item=$(this);
		if($("a",this).html().match(/Private/)) item.remove();
	});
	}
    ...
    var current=parseInt($(".area_paging span.selected").text());
		...
		if(next>max) next="";
			...
			var prev_link=$("<a>").attr("href","?page="+prev).text("이전");
			var next_link=$("<a>").attr("href","?page="+next).text("다음");
			var current_link=$("<a>").attr("onclick","expand_pages()").text(current+"/"+max);
		...
		var all_links=$("<div>").attr("id","all_links").html("<table><tr></tr></table>");
			...
			for(var i=1;i<=max;i++){
				var link=$("<a>").attr("href","?page="+i).text(i);
